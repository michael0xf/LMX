...
- Все дальнейшие реплики автора сохранять в `blog/` в порядке поступления, без исправления слов и без пересказа. Отделять цитаты автора от комментариев агента. Журнал не требует перевода: это первичные сообщения, а не парная спецификация.
