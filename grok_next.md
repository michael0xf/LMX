...
9. **§7 порт implements/admission** — текущий пункт. T5–T7 посажены, REVIEW c713873 OK. План `steps/implements-s7.md`. Identity: `unit_s7_identity` Entry 7. Пустой `uses`: Consumer без полей, `unit_s7_empty` Entry 7. Непустой `uses`: Consumer — прочитанные пути (`l2_descriptor_used`), `unit_s7_used` Entry 7 (`Plain` без `extra` допускается, раз `extra` не читали). Прочитанного поля нет — прежний отказ. Таблица `primitive.convert` — данные Message, отбор не по имени, глобальный реестр не заводится. §8-blocking.
