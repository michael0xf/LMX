...
1. **Результат вызова как актуал — ложный допуск.** `fn: mk () Plain` (`Plain{toString}`), `fn: take (Equatable: slot) int` с телом `slot\equals: 1U`, вызов `take(mk())` — переводится, exit 0, в переводе нет ни `lmx_runtime_implements`, ни `admit` (единственное упоминание — predef `l2src/lmx_implements.h.lm1`; проба `arg_call.lm2`, knob off и `--walk-methods`). Тот же `take(Plain)` атомом отказывается «implements is false in function argument» (`unit_invalid_implements_used_field`). Это обход §7 :447 и нарушение нормы автора (`LMX_blog/2026-09-22.md` :55: присвоение известных всегда вызывает `implements`).
