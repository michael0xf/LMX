...
- **links:** requires=see related articles; produces=see definition; consumes=see definition; selects=see definition
- **authoritative sources:** next_core_tasks.md; AUTHOR tickets 20260922; LMX_blog 2026-09-22 where applicable
- **implementation (files/functions):** the record is **`struct: LmxArrayDesc` at `dev/l2src_sandbox/lmx.h.lm1:203-206`**, exactly two fields — `size_t: len` and `@: void data`. (Declared in `lmx.h.lm1`; a search of `lmx_array_owned.h.lm1` finds nothing, which is a convenient place to lose a minute.) **`LmxListDesc` no longer exists as a header type: 0 occurrences in the live tree** — dynamic membership is `KIND_LIST` over this same `{len, data}` layout with `cap` in the backing prefix. Landing commits: `dc57747` ("Restore LmxArrayDesc to {len, data}; L1 list for dynamic membership"), `7419470`, `f0afb6a`.
