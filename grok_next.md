...
7. **Дефекты открытые** (`steps/defects.md`): **D-62 ПОСАЖЕН** (`unit_send_ref_root_fail`: abort, `lmx: invariant: sendMessage failed`). **D-12 ПОСАЖЕН** (`printTree.lm2`: корень и `mainArgs`, `@document` в методе; пример L2 §18.2). **D-39 ПОСАЖЕН (F-72):** индекс ELEM/ELEMPUT — вычисленный `size_t` (не сырая ячейка и не тихий каст из `int`); `stack\columns[idx]` — индекс `c.*`, не own-массив. D-36 не сажать прямым вызовом (`lmx_call_install_walk` — шов компоновки). Дальше D-19/D-20/D-23/D-24/D-27 (транслятор), D-09 (S1.5), D-17 (инфра), D-25 (учёт фикстур).
