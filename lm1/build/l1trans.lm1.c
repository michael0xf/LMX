...
int lm_p0_normalize_sole_anonymous_container(LmP0Structure * body);
int lm_p0_normalize_sole_anonymous_in_trailer(LmP0Trailer * trailer);
int lm_p0_normalize_sole_anonymous_in_node(LmP0Node * node);
int lm_p0_normalize_sole_anonymous_in_structure(LmP0Structure * structure);
...
int lm_p0_record_mix_mark(LmP0Document * document, LmP0Structure * structure, const char * text, size_t length, size_t line, size_t column, size_t start, size_t end)
{
    ...
    if (lm_p0_normalize_sole_anonymous_in_node(payload_document->root) == 0) {
    lm_p0_copy_payload_diagnostic(document, payload_document, payload_offset);
    }
    else {
...
int lm_p0_stream_resolve_pending_delimiter(LmP0Document * document, LmP0Stack * stack, LmP0PendingDelimiter * pending, size_t next_level)
{
    ...
    if (empty_colon) {
    trailer_slot[0]->flags = trailer_slot[0] -> flags | LM_P0_TRAILER_DELIMITER_CLOSED;
    }
    }
    else {
    ...
    if (empty_colon) {
    owner->as->frame->flags = owner -> as -> frame -> flags | LM_P0_FRAME_DELIMITER_CLOSED;
    }
    }
    }
    }
...
int lm_p0_normalize_sole_anonymous_container(LmP0Structure * body)
{
    ...
    if (body -> field_count != 1U) {
    return 1;
    }
    only = body -> first_field;
    ...
    inner = container -> as -> structure;
    if (inner -> trailer != 0) {
    return 1;
    }
    body->first_field = inner -> first_field;
    body->last_field = inner -> last_field;
    body->field_count = inner -> field_count;
    inner->first_field = 0;
    inner->last_field = 0;
    inner->field_count = 0U;
    only->value = 0;
    only->next = 0;
    return 1;
}
int lm_p0_normalize_sole_anonymous_in_trailer(LmP0Trailer * trailer)
{
    ...
    if (lm_p0_normalize_sole_anonymous_in_structure(trailer->body) == 0) {
    return 0;
    }
    return lm_p0_normalize_sole_anonymous_container(trailer->body);
}
int lm_p0_normalize_sole_anonymous_in_node(LmP0Node * node)
{
    if (node == 0) {
    ...
    if (node -> kind == LM_P0_NODE_FRAME && node -> as != 0 && node -> as -> frame != 0) {
    if (lm_p0_normalize_sole_anonymous_in_structure(node->as->frame->body) == 0) {
    return 0;
    }
    if (lm_p0_normalize_sole_anonymous_container(node->as->frame->body) == 0) {
    return 0;
    }
    return lm_p0_normalize_sole_anonymous_in_trailer(node->as->frame->trailer);
    }
    if (node -> kind == LM_P0_NODE_STRUCTURE && node -> as != 0) {
    if (lm_p0_normalize_sole_anonymous_in_structure(node->as->structure) == 0) {
    return 0;
    }
    return lm_p0_normalize_sole_anonymous_in_trailer(node->as->structure->trailer);
    }
    return 1;
}
int lm_p0_normalize_sole_anonymous_in_structure(LmP0Structure * structure)
{
    ...
    field = structure -> first_field;
    while ((field != 0)) {
    if ((lm_p0_normalize_sole_anonymous_in_node(field->value) == 0)) {
    return 0;
    }
    field = field -> next;
    }
...
int lm_p0_validate_nonempty_colon_frames_in_trailer(LmP0Document * document, const LmP0Trailer * trailer)
{
    ...
    if (lm_p0_colon_trailer_empty(trailer) != 0 && ((trailer -> flags & LM_P0_TRAILER_DELIMITER_CLOSED) == 0U)) {
...
int lm_p0_validate_nonempty_colon_frames_in_node(LmP0Document * document, const LmP0Node * node)
{
    ...
    if (node -> kind == LM_P0_NODE_FRAME && node -> as != 0 && lm_p0_colon_frame_empty(node->as->frame) != 0 && ((node -> as -> frame -> flags & LM_P0_FRAME_DELIMITER_CLOSED) == 0U)) {
    lm_p0_set_diagnostic(document, 32, line, column, "empty colon Frame is not allowed");
    return 0;
    }
    if (node -> kind == LM_P0_NODE_FRAME && node -> as != 0 && lm_p0_colon_trailer_empty(node->as->frame->trailer) != 0 && ((node -> as -> frame -> trailer -> flags & LM_P0_TRAILER_DELIMITER_CLOSED) == 0U)) {
    lm_p0_set_diagnostic(document, 32, line, column, "empty colon Frame is not allowed");
    return 0;
    }
    if (node -> kind == LM_P0_NODE_STRUCTURE && node -> as != 0 && lm_p0_colon_trailer_empty(node->as->structure->trailer) != 0 && ((node -> as -> structure -> trailer -> flags & LM_P0_TRAILER_DELIMITER_CLOSED) == 0U)) {
    lm_p0_set_diagnostic(document, 32, line, column, "empty colon Frame is not allowed");
    return 0;
    }
    if ((node -> kind == LM_P0_NODE_FRAME)) {
...
int l1_emit_extern_decl(FILE * out, const LmP0Frame * frame, const char * path);
...
int l1_is_type_name(const LmP0Text * text)
{
    ...
    if (l1_text_eq(text, "uintptr_t") || l1_text_eq(text, "intptr_t")) {
    return 1;
    }
    if (l1_text_eq(text, "uint_fast8_t") || l1_text_eq(text, "uint8_t") || l1_text_eq(text, "uint16_t")) {
    return 1;
    }
    if (l1_text_eq(text, "uint32_t") || l1_text_eq(text, "uint64_t")) {
    return 1;
    }
    if (l1_text_eq(text, "int8_t") || l1_text_eq(text, "int16_t") || l1_text_eq(text, "int32_t") || l1_text_eq(text, "int64_t")) {
    return 1;
    }
...
int l1_is_keyword(const LmP0Text * text)
{
    ...
    if (l1_text_eq(text, "thread_local")) {
    return 1;
    }
    return 0;
}
...
int l1_emit_param(FILE * out, const LmP0Node * node, const char * path)
{
    ...
    if (extra -> value == 0 || extra -> value -> kind != LM_P0_NODE_ATOM) {
    return l1_error(path, node, "const parameter missing name");
    }
    if (l1_write_cstr(out, " ") != 0) {
    return 1;
    }
    if (l1_write_ident(out, extra->value->as->atom, path, extra->value) != 0) {
    return 1;
    }
    return 0;
    }
...
int l1_emit_inits(FILE * out, LmP0Field * start, const char * path)
{
    ...
    if (field -> value -> kind == LM_P0_NODE_FRAME && l1_text_eq(field->value->as->frame->head, "cast")) {
    if (l1_emit_cast(out, field->value->as->frame->body, path) != 0) {
    return 1;
    }
    }
    else {
...
int l1_emit_thread_local(FILE * out, const LmP0Frame * frame, const char * path, const LmP0Node * node)
{
    LmP0Field * field;
    const LmP0Node * child;
    const LmP0Text * ch;
    int saw = 0;
    if (frame == 0) {
    return l1_error(path, node, "thread_local expects declarations");
    }
    if (l1_validate_end_trailer(frame, path, node) != 0) {
    return 1;
    }
    ...
    if (l1_node_ignored(child) == 0) {
    if (child == 0 || child -> kind != LM_P0_NODE_FRAME) {
    return l1_error(path, child, "thread_local expects declarations");
    }
    ch = child -> as -> frame -> head;
    if (l1_text_eq(ch, "const") || l1_pointer_depth(ch) != 0U || l1_is_type_name(ch) || l1_text_eq(ch, "foreign")) {
    if (l1_write_cstr(out, "    _Thread_local ") != 0) {
    return 1;
    }
    if (l1_emit_data_decl(out, child, path, 0) != 0) {
    return 1;
    }
    }
    else {
    ...
    if (l1_emit_bracket_array(out, child->as->frame, path, 0) != 0) {
    return 1;
    }
    }
    else {
    return l1_error(path, child, "thread_local does not qualify this form");
    }
    }
    saw = 1;
    }
...
int l1_emit_stmt(FILE * out, const LmP0Node * node, const char * path)
{
    ...
    if (l1_text_eq(head, "thread_local")) {
    return l1_emit_thread_local(out, frame, path, node);
    }
...
int l1_unit_item_ok(const LmP0Text * text)
{
    ...
    if (l1_text_eq(text, "struct") || l1_text_eq(text, "enum") || l1_text_eq(text, "fnptr") || l1_text_eq(text, "union") || l1_text_eq(text, "type") || l1_text_eq(text, "foreign") || l1_text_eq(text, "extern")) {
    return 1;
    }
...
int l1_validate_implicit_node(const LmP0Node * node, const char * path, int top)
{
    ...
    if (top != 0 && l1_text_eq(node->as->frame->head, "foreign")) {
    if (l1_hdr_frame_name(node->as->frame) == 0) {
    return l1_error(path, node, "foreign type missing name");
    }
    return l1_hdr_type_add(l1_hdr_frame_name(node->as->frame), path, node, 103);
    }
...
int l1_hdr_mark_unit_emitted(const LmP0Node * root, const char * path)
{
    ...
    if (l1_text_eq(node->as->frame->head, "foreign")) {
    name = l1_hdr_frame_name(node->as->frame);
    }
    if (name != 0) {
...
int l1_emit_item(FILE * out, const LmP0Node * node, const char * path, int in_l1, int depth)
{
    ...
    if (l1_text_eq(node->as->frame->head, "extern")) {
    return l1_emit_extern_decl(out, node->as->frame, path);
    }
    if (l1_text_eq(node->as->frame->head, "foreign")) {
    return 0;
    }
    ...
    if (l1_pointer_depth(node->as->frame->head) != 0U || l1_text_eq(node->as->frame->head, "const") || l1_is_type_name(node->as->frame->head) || l1_text_eq(node->as->frame->head, "c.array") || l1_text_starts(node->as->frame->head, "[]") || l1_text_eq(node->as->frame->head, "immutable") || l1_text_eq(node->as->frame->head, "thread_local")) {
...
int l1_emit_extern_decl(FILE * out, const LmP0Frame * frame, const char * path)
{
    LmP0Field * field;
    if (frame == 0 || frame -> body == 0) {
    return l1_error(path, 0, "extern declaration missing");
    }
    field = frame -> body -> first_field;
    while (field != 0 && l1_node_ignored(field->value)) {
    field = field -> next;
    ...
    if (l1_write_cstr(out, "extern ") != 0) {
    return 1;
    }
    return l1_emit_data_decl(out, field->value, path, 0);
}
...
int l1_hdr_check_item(const LmP0Node * node, const char * path)
{
    ...
    if (l1_text_eq(frame->head, "foreign")) {
    name = l1_hdr_frame_name(frame);
    if (name == 0) {
    return l1_error(path, node, "foreign type missing name");
    }
    return l1_hdr_type_add(name, path, node, 103);
    }
    if (l1_text_eq(frame->head, "extern")) {
    return 0;
    }
    if (l1_text_eq(frame->head, "thread_local")) {
    field = 0;
    if (frame -> body != 0) {
    field = frame -> body -> first_field;
    }
    ...
    ch = field -> value -> as -> frame -> head;
    if (l1_text_eq(ch, "const") || l1_pointer_depth(ch) != 0U || l1_is_type_name(ch) || l1_text_eq(ch, "foreign") || l1_text_eq(ch, "[]") || l1_text_starts(ch, "[]")) {
    return 0;
    }
    return l1_error(path, node, "thread_local does not qualify this form");
    }
    ...
    if (alias_after_tag != 0) {
    alias_after_tag = 0;
    }
    else {
    if (l1_text_eq(field->value->as->atom, "enum") != 0 || l1_text_eq(field->value->as->atom, "struct") != 0 || l1_text_eq(field->value->as->atom, "union") != 0) {
    alias_after_tag = 1;
    }
    else {
