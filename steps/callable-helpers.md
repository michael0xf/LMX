...
- В дескрипторе: ссылка на нативный `LmxMethod` и дерево дескриптора (`header`). Тело — базовое дерево M после слота 0, по порядку; все `return:` там.
...
Сделано в песочнице следом: `LmxMethod` — первый член `LmxCallable`; `lmx_call_method` ветвится по виду; `lmx_callable_new_owned`; selftest `tests/lmx_callable_selftest.lm1`. `return_arg` с дескриптора снят.

## Ещё открыто

- Представление узлов тела (операторы и все `return:`) в графе по порядку — следующий шаг `l2trans`, не одиночный слот на дескрипторе.
