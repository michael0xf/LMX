...
9. **§7 порт implements/admission** — текущий пункт. T5–T7 посажены, REVIEW c713873 OK. План `steps/implements-s7.md`. Допуск по `uses` — список путей в `l2_descriptor_used`: ноль путей успех (`unit_s7_empty` Entry 7), прочитанные имена сверяются (`unit_s7_used` Entry 7, непрочитанный `extra` не требуется; нет прочитанного поля — отказ). 64 пути — Entry 7 (`unit_s7_uses64`); 65-й — «a uses list is full». Равных индексов это тоже касается (`unit_s7_identity` Entry 7). Кадра нет — прежний дескриптор. Первый сегмент пути; вложенное имя — грубость S3. Таблица `primitive.convert` — данные Message, отбор не по имени, глобальный реестр не заводится. §8-blocking.
