...
`struct: LmxArrayDesc` is exactly `{len, data}` with no `Lmx` node: `len` is the logical element count and `data` is the address of a backing that has exactly `len` cells. Base Array has no `capacity`, `reserve`, `resize`, `append`, or grow, and does not switch or compact backing. Dynamic membership, capacity, and growth belong only to a separate List (`KIND_LIST`) whose capacity lives in a private backing prefix, not on `LmxArrayDesc`. Length belongs to array elements, not Structure fields ([semantics](LMX_semantics.en.md#values)). L1: [L1](L1_spec_en.md#method-array).
