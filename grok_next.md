...
4. **-200 POSIX-двойник** — срезы (а) и (б) ПОСАЖЕНЫ (`7cd11f3`, `c4fe4d8`; машина 279/398/11; облако fable: `build_l2src.py` 182/232, L3 6/11). **Остаток — REVIEW b5e72d7 п.1:** `WaitForSingleObject` в `lmx_thread.lm1:872` (`lmx_thread_lane_ended`) и `c.Sleep` в `lmx_domain_selftest.lm1:302` → в контракт lane общего заголовка, реализация в обоих телах; цель 232/232 и 11/11 в облаке, fable измерит и запишет. Дальше по плану `steps/posix-200.md`.
5. **T4b** — класс 1 (Structure-формал) ПОСАЖЕН (`steps/walk-methods-t4b.md`: путь `path[0] = 6` это ARG j, поле OF, запись PUT_OF; свидетель Entry 7; дифф `--walk-methods` 215/215; harness 399/399). **Следующий код — не класс 2, а остаток п.4 (REVIEW b5e72d7-1).** Класс 2 (formal binding) после него. Дальше own-поля во вложенном теле; throws и callable-формалы ждут своего места.
