...
- [28. Ordinary LMX calls over Message transport: `post` and reply waits](#thread-message-api)
- [29. Mix: active marks and intervals](#mix)
- [30. WorldWideMix: addresses, cells and navigation](#worldwide)
- [31. Prefix coordination and mirrors](#mix-coordination)
- [32. HTTP and service boundaries](#transport)
- [33. Authentication, authority and evidence](#authentication)
- [34. Cryptographic values and providers](#crypto-values)
- [35. Cryptographic operation contracts](#crypto-operations)
...
<a id="thread-message-api"></a>
## 28. Ordinary LMX calls over Message transport: `post` and reply waits
...
2. **Identity and context of the source message: as with `merge`.** Author: the whole used context travels by the same mechanism as internal calls, "as if we had a reference to another graph"; further narrowing (something Independent is already being cut) comes as separate decisions, the principle of one shared mechanism stays. Consequence: the transport defines no identity contract of its own; correlating a reply with its request is a reference to the source message, not a key in a second dictionary.
...
4. **The clock is the existing one.** The timestamp is the moment the send starts (author). The source is the kernel clock `lmx_clock_now`/`lmx_deadline_passed` (`lmx_clock.h.lm1`, `lmx_clock_posix.lm1`/`_win32.lm1`); the wait deadline is the send timestamp plus `timeout\ms\time`; it is checked at the sender's processing points, with no second timer or thread per `post`; expiry calls the user's `unanswered` (§6).
...
<a id="worldwide"></a>
## 30. WorldWideMix: addresses, cells and navigation
...
## 31. Prefix coordination and mirrors
...
<a id="transport"></a>
## 32. HTTP and service boundaries
...
## 33. Authentication, authority and evidence
...
## 34. Cryptographic values and providers
...
## 35. Cryptographic operation contracts
