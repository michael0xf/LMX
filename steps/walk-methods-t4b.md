...
Поле заводит только объявление (D-79, автор). Голое присваивание формалу поле тела не заводит: первая запись связывает ячейку метода (L3 §12), последующая — в том числе из `if` — пишет в неё. Выход из тела снимает отметку только у поля, объявленного в этом теле (`l2_emit_body` / `l2_rw_leave`). Отметка формала, поставленная до тела или голой записью в ячейку метода, не снимается. Счётчик `l2_rw_bseq_n`; проход подсчёта кадров (`l2_rw_count`) откатывает отметки, иначе эмиссия видела бы поле уже связанным на его строке. Корень область `if`/`while` не открывает: повторный `receiveMessage: m` в `while` остаётся полем корня (`unit_next_message_loop`).
...
- `keep(3)` = 4 (D-79; прежние 3 — дефект): `int: n` заводит поле метода и переносит 3, `n: n + 1` внутри `if` пишет ту же ячейку;
- `branch(3)` = 43: `int: n` внутри `if` — объявление, поле этого тела; после тела `n` — формал (машинный аргумент 3), поля метода нет (`CORE.md` §3).
...
## D-79 — поле заводит только объявление

Поправка автора к классу 3 (REVIEW 3f68f41, BLOCK). Оба пути в `l2trans.lm1`. Ядро не менялось.

Голое присваивание ищет строку в текущем теле, затем ячейку метода (host 0). Нет ни той, ни другой — первая запись заводит ячейку метода, не поле тела, в котором она стоит. Объявление в теле (`int: n` в `if`) по-прежнему поле этого тела. `l2_emit_body` и `l2_rw_leave` снимают отметку только у поля, чей host — это тело. Ветка walk «assignment whose name is a field of the nested body» снята: запись в объявленное поле тела идёт через `l2_rw_own_row` / `l2_rw_formal_slot`.

Свидетели, нативно и под `--walk-methods`, Entry 7:

- `unit_walk_nested_own`: `keep(3)` = 4, `branch(3)` = 43, `nest` 43;
- `unit_arg_bind_body_scope`: `scoped(3, 1)` = 77, `nested(3)` = 1414.

Строки c3c (`unit_occ_arg_slots`, `unit_occ_root_named`, `unit_occ_snapshot_selector`) факты не сменили: первая голая запись на уровне метода по-прежнему одна ячейка, `\[N]` считает объявления.

Мутант: `l2_collect_asgn_body` снова зовёт `l2_own_add` в текущем теле. `unit_walk_nested_own` нативно — exit 5, ожидался 7 (`keep(3)` снова не 4). Откат — Entry 7.
