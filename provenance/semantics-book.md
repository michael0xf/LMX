...
Вход в метод не копирует его вызываемое вхождение, тело или другую часть графа. Нативное и интерпретируемое исполнение используют одну модель: исполнение — пара из кода и данных, где роль задаётся позицией (Structure в позиции головы — код, переданная ей Structure — данные), а не свойством Structure. Код неизменен во время исполнения; объявленные в теле поля образуют прототип данных метода, и каждая активация получает свежий экземпляр этого прототипа, созданный полем данных лексического родителя в слоте этого вызываемого вхождения; фактические аргументы записываются в его поля, а формальные, временные значения и результат живут в обычном кадре со временем жизни стекового вызова. Рекурсивный вызов создаёт другой экземпляр и кадр над тем же методом; слот родителя показывает экземпляр последней активации. Реализация может размещать служебное хранилище кадра в переиспользуемой памяти, если это не создаёт копию метода или скрытое состояние графа и не делает вход существенно дороже нативного вызова; память графа копируется только явно выраженной операцией, а не самим вызовом.
...
Entering a method does not copy its callable occurrence, body or any other part of the graph. Native and interpreted execution use the same model: an execution is a pair of code and data, where the role is given by position (the Structure in head position is code, the Structure passed to it is data), not by a property of the Structure. Code is immutable during execution; the fields declared in the body form the method's data prototype, and each activation receives a fresh instance of that prototype, created as a field of the lexical parent's data in the slot of this callable occurrence; the actual arguments are written into its fields, while formal and temporary values and the result live in an ordinary frame with the lifetime of a stack call. A recursive call creates another instance and frame over the same method; the parent's slot shows the instance of the latest activation. An implementation may place the frame's auxiliary storage in reusable memory provided that this neither creates a method copy nor hidden graph state and does not make entry substantially more expensive than a native call; graph memory is copied only by an explicitly expressed operation, not by invocation itself.
...
Собственное поле текущей активации — поле её экземпляра данных; присваивание такому имени пишет в это поле непосредственно, рабочих копий и отметки изменения нет. Явная запись через ссылку меняет выбранный объект непосредственно и наблюдаема через другие ссылки на него. Явный или скрытый аргумент — поле того же экземпляра; без направленного в него голого присваивания он остаётся обычным входным значением без обратной записи вызывающему выражению.

Наличие в исходном теле разрешённого голого присваивания `x: value`, где `x` — заранее типизированный изменяемый явный либо скрытый аргумент, делает `x` полем прототипа данных этого тела. Для роли присваивания `value` также должно быть разрешено как существующее типизированное значение либо непосредственно как литерал; иначе поле по этому правилу не создаётся, а выбирается другая уже определённая роль двоеточия либо выдаётся ошибка трансляции. Исполнение строки записывает значение в поле экземпляра; не взятая ветка ничего не записывает. Трансляция уже зафиксировала раскладку прототипа, поэтому исполнение не меняет число полей. Запись попадает в экземпляр данных этого тела, никогда — в источник аргумента вызывающего выражения или в родительский граф.

Локальность и длительность хранения здесь являются разными свойствами. Экземпляр данных последней активации остаётся в слоте вызываемого вхождения и доступен снаружи путём `M\x`, но запись в него не является изменением внешней привязки: ячейка аргумента вызывающего выражения и поле над-методного пространства остаются неизменными. Это внешнее поле меняется только явной записью через путь `node\x`, где `node` — данные лексического родителя.
