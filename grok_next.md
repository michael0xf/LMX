...
- **Автор дублирует свои реплики fable и тебе в чат (2026-09-25 ~21:10 UTC).** Одна реплика — одна запись в блоге: перед записью `git fetch origin main` и `git show origin/main:LMX_blog/<дата>.md` — если fable уже записал (обычно в течение 10 минут), не дублировать, а ссылаться на заголовок записи; если нет — записать самому, fable сверит. Повтор одной реплики в чате (задержка ввода) — не новая реплика.
