...
2. ~~**-203 массивы над вычисленным операндом**~~ — ПОСАЖЕН (`901ae2a` + `cab153c` + `ab1ecbe`; одна форма, `entry_argc_if` бежит с Entry 7, мутации измерены; REVIEW 901ae2a/8f8ad3a/ab1ecbe). Остаток класса: `c.puts`/`c.strcmp` в корне ([q31](LMX_blog/q/current/q31.md)), `size_t → int` (конверсии §7). D-39 (индекс) ПОСАЖЕН, F-72. Ниже — прежнее описание: ELEM/ELEMPUT/LENGTH берут операнд-узел, дающий дескриптор массива (OF/DEREF/ELEM массива массивов), классификация по арене, **одна форма** (эмиссия 5a переводится на неё; 5 строк 5a — регрессионный пин); даёт 5 строк 5b (`length(m\mainArgs)`, `m\mainArgs[1][0]`); адреса `@ m\mainArgs[1][0]` в C-вызовах — [q31](LMX_blog/q/current/q31.md), отдельно. Ядро (`lmx_walk.lm1` :548-:571, армы 25–27) + транслятор (`l2_rw_elem*`, `l2_rw_opty`).
...
10. **§1 миграция `Lmx` → `{VoidArray array; Lmx *parent; LmxEntry native}`** — открытый вопрос [q32](LMX_blog/q/current/q32.md), не закрытый Q9 из `q.md`. До ответа код раскладки не писать.
...
- Открытые вопросы — `LMX_blog/q/current/`: [q31](LMX_blog/q/current/q31.md) (корень файла — independent; отказ «outside a method body» — остаток), [q32](LMX_blog/q/current/q32.md) (раскладка `Lmx`). Изменение фактов harness, вызванное нормой, а не дефектом.
