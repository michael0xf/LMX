...
- Current `l2_collect_asgn_body` restricts argument-as-own creation through a repeated `int`/`char`/`size_t`/`ulong` type list even though later publication already has `unsigned` and pointer branches. `l2_emit_checkpoint` clears every dirty mark without a state for “input address taken before binding.” This differs from the type-independent, temporally ordered [L3 §11](LMX_semantics.en.md#dynamic) contract rather than limiting the language; separate before-bind, never-bound and after-bind RED cases are required until it is fixed.
