...
- [ ] Гейты до кода: selftest раскладки (размер/смещения `Lmx`, `VoidArray`; **свидетель смещения 0: `c.offsetof(Lmx, array) = 0`**, `parent` следом; имени `LmxArrayDesc` в дереве нет — проверка `rg` в гейте), свидетель раздельной идентичности диапазонов (`lmx_range_classify` для вложенного `Lmx.array` backing и для отдельного Array даёт `LMX_KIND_ARRAY`, для самого `Lmx` — `LMX_KIND_STRUCT`; мутант «дескриптор как LmxRange» — RED), строки harness, пиннящие чтения `node\array\size`/`\data` в сгенерированном C, self-build 8/8 на seeds; мутанты (лишнее поле в `Lmx`; `int` вместо `size_t`; чтение `len` мимо вложенности) — RED.
