...
8. **K3 + T5–T7** (`steps/merge-parts-193.md` §2 (1e)(1f)(2f)(2g)(3)): callable merge по частям (args ← args, return ← return, body ← body; операторы тела последнего операнда с телом заменяют модель), PAP `add5: merge(y: 5; add)` → 6 через `lmx_call_prim`, makeAdder, конвертер «Structure с callable → callable». Свидетели: q22 → 3, q20-next §2 → 5 (`LMX_blog/q/`). **T5 срез 1 ПОСАЖЕН** (`07115c6`, main `24ac536`): `unit_pap_add5` Entry 6, harness 414/414 (`build/l2_harness/pap_t5b`); build и L3 на посадке не гонялись. Дальше T6, план `steps/callable-merge-t6.md`.
