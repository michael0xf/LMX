...
| D-03 | 2026-09-24, Opus -138 | L3 `l3_exec_stmt` :241/:245 — голый CALLABLE-оператор, plain Structure-оператор и примитив дают молча `L3_OK`; walker `lmx_walk_body` :809–:821 пропускает голую callable-ссылку в теле молча — норма N7 требует исполнения или явного UNSUPPORTED | Grok | OPEN → -140 |
| D-04 | 2026-09-23, Opus -133 | Walker не проверяет число аргументов на входе: лишний Structure-аргумент нульарному callee интерпретатор не отвергает (native отвергает) | Grok | OPEN → -140 |
