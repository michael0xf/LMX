...
Двоеточие создаёт одну общую форму «голова принимает хвост»; роль головы определяет контекст после разрешения привязок. В `head: tail` само двоеточие не объявляет, не присваивает и не вычисляет `tail`. В частности, `u32: id`, `Text: crop`, `f64: harvest 0.0` являются объявлениями только когда следующий идентификатор ещё не имеет активной логической привязки в этой точке: тогда разрешённая операция-конструктор принимает его как необработанное предлагаемое имя. Одноимённое заранее подготовленное физическое место не считается активной привязкой. Если значение хвоста уже существует, хвост вычисляется, а роль головы выбирается для этого контекста; конструкторное толкование не имеет безусловного приоритета.

В контексте переменной/конструктора развилка `left: right` определяется активной привязкой `right`, а не написанием имён. Если `right` отсутствует, это контекст объявления: `left` обязан разрешиться как операция типа/модели, хвост передаётся ей как необработанное предлагаемое имя, а ветвь `right` создаётся настоящим полным `merge(left, empty)`. Другого механизма создания ветви нет. Неквалифицированный `left` выбирается по обычной динамической видимости: текущее локальное значение вызывающего, затем унаследованный динамический вход и только затем лексический fallback вызываемого. Поэтому транслятор знает требуемый контракт, но не обязан знать физическую идентичность фактического операнда. Явный `node\left` выбирает именно лексический объект, минуя динамическое замещение. Пустой второй операнд не сокращает обход до копирования указателя: общая операция `merge` сохраняет одну карту «источник — копия», обходит полное используемое замыкание, переписывает ссылки и структурный `parent` и применяет своё терминальное правило. Встретив явно выбранный `independent: const: immutable`, она сохраняет исходную физическую ссылку без копирования и смены владельца. Внешний `const:` фиксирует полученную привязку. Поэтому `env: field` при отсутствующем `field` и `field: env2` при отсутствующем `env2` являются одной ситуацией объявления с разными именами.

Если `right` уже имеет активную привязку, он вычисляется как кандидат-значение, а `left` является целью обновления. Цель `left` должна уже разрешаться в изменяемую привязку с заранее известным типом; отсутствующая либо нетипизированная цель и `const left` являются ошибками. До любой записи выполняется полный `implements(right, left, Consumer)`, включая аналитическую проверку и обязательные юнит-тесты принимающего выражения; при неуспехе цель и её состояние `dirty` не изменяются. Только после успеха в `left` присваивается ссылка `right`: это не `merge`, ветвь не копируется и владелец целевого значения не меняется. Присваивание не выводит тип цели из правой части. Поэтому `arg: 7` допустимо при заранее типизированном явном **или скрытом** аргументе `arg`, но недопустимо в теле без независимо установленной типизированной привязки `arg`. Кроме того, правая часть должна быть разрешена транслятором как известное значение: литерал известен непосредственно, а имя должно разрешаться в типизированную привязку доступного namespace. Транслятор обязан знать тип и место привязки, хотя физическая ссылка динамического значения может стать известна лишь при вызове. Лишь при выполнении обоих условий эта контекстная роль двоеточия является присваиванием: транслятор подготавливает одноимённое own-поле тела, а исполнение связывает с ним локальный кэш и ставит `dirty`. Если условия присваивания не выполнены, это own-поле не возникает: выбирается иная уже определённая контекстная роль двоеточия или выдаётся ошибка трансляции. Это не объявляет переменную заново и не изменяет источник аргумента у вызывающего выражения.
...
Для такой привязки действует одно узкое адресное правило. Если `@x` было вычислено **до** первой исполненной строки `x: ...`, создающей эту привязку, то с момента исполнения строки отметка связанного own-поля становится липкой до конца текущей активации. Каждая последующая контрольная точка публикует текущее локальное `x`, но успешная публикация не очищает эту отметку. Правило не зависит от типа `x`: оно применяется к любому типу, для которого само присваивание-объявление допустимо.

| Порядок в текущей активации | Результат |
| --- | --- |
| `@x`, затем исполняется первое связывающее `x: ...` | С момента привязки `dirty` липкий до конца активации |
| `@x`, но связывающее `x: ...` не исполняется | `x` остаётся обычным локальным входом; графового `dirty` нет |
| Сначала исполняется связывающее `x: ...`, затем вычисляется `@x` | Взятие адреса не делает уже связанную отметку липкой; обычный `dirty` очищается контрольной точкой |

Это правило не распространяется на обычные локали, обычные graph-backed own-поля, явные пути вроде `node\x` и элементы массива. Адрес входа остаётся обычным адресом локальной ячейки и не меняет источник у вызывающего выражения. Сохранение такого адреса после завершения активации недопустимо независимо от `dirty`.
...
Общий физический дескриптор Array содержит `len`, `capacity` и `data`: логическое число элементов, число ячеек текущего backing и его адрес. Для фиксированного или неизменяемого массива `capacity = len`. Растущий массив ссылок начинает с ёмкости 2 и увеличивает её приблизительно в `3/2` раза. Рост или компактация могут заменить backing, поэтому адрес его отдельной ячейки не сохраняется через такую операцию; физические адреса объектов, хранимые как значения ссылок, от этого не меняются.
...
Ребёнок проверяет жизнеспособность родителя и после длительного отсутствия ответа начинает собственное упорядоченное закрытие. Принудительное закрытие сверху — аварийный второй путь. Родитель обслуживает физических адресатов из единственного графового массива своих прямых детей и не выполняет произвольное управление внуками. Каждый ребёнок повторяет каскад для собственного массива; закрытие распространяется по семейству, а не сохраняет навсегда освобождённую ветвь.
