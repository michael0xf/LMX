...
9. **§7 порт implements/admission** — текущий пункт. T5–T7 посажены, REVIEW c713873 OK. План `steps/implements-s7.md`. Срез 1: identity вызывает `l2_descriptor_implements` и при `cand = req` остаётся успехом (`unit_s7_identity` Entry 7). Пустой `uses` идёт в тот же вызов с Consumer без полей (`l2_ns_n`, `cons_n = 0`): `unit_s7_empty` Entry 7, `Plain` допускается в `Equatable`. Непустой `uses` по-прежнему передаёт `req`. Таблица `primitive.convert` — данные Message, отбор не по имени, глобальный реестр не заводится. §8-blocking.
