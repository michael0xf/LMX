...
`LmxArrayDesc` is an alias for the same `VoidArray {size_t size, void *data}` without `parent`: a standalone Array descriptor and the embedded `Lmx.array` have one physical type. For a standalone Array, `size` counts its elements and its backing contains exactly that many cells; for `Lmx.array` it counts child slots. The standalone Array descriptor and a Structure header occupy different typed arena ranges even though the embedded array has the same layout. Base Array has no `capacity`, `reserve`, `resize`, `append`, or grow, and does not switch or compact backing. Dynamic membership, capacity, and growth belong only to a separate List (`KIND_LIST`) whose capacity lives in a private backing prefix, not in the base descriptor. Length belongs to array elements, not Structure fields ([semantics](LMX_semantics.en.md#values)). L1: [L1](L1_spec_en.md#method-array).
