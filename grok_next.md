...
9. **§7 порт implements/admission** — текущий пункт. T5–T7 посажены, REVIEW c713873 OK. План к.1: `steps/implements-s7.md`. Первый срез — identity проходит через вызов `l2_descriptor_implements`, результат успеха тот же. Пустой `uses` остаётся успехом: жирный обход полей required на этом пути добавил бы отказ. Таблица `primitive.convert` — данные Message, глобальный реестр не заводится. §8-blocking.
