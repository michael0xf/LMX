...
`lmx_call.h.lm1`: callability is determined by the address range of the stored `child[0]` value. The new Callable path passes two physical references to a native entry: reserved source `node = callable\parent`, the above-method space, and hidden own-field base `self = callable`; `self` is not a language word. The `offset` of the lexical parent's fields inside `node` is not passed as an argument: the prologue reads it from the descriptor `self\child[0]` ([§13](#copy-merge)). Transitional direct METHOD keeps the old one-argument entry only for legacy low-level targets. This stage performs **no** signature check (as in C; full typing is separate), and `sig` is not decoded. L1: [L1](L1_spec_en.md#call).
...
Modules `lmx_graph_copy_owned` and `lmx_merge_owned`. Traversal matches Message creation: the complete used closure is copied, while `parent` and other references are rewritten through one source-to-copy map; METHOD pointers are retained. Operand fields are appended to the result flat, in operand order; an unqualified name reads the last occurrence, `[N]` the lexical occurrence number ([semantics](LMX_semantics.en.md#fields)). Methods are not merged: a callable occurrence is copied as a subtree, its `parent` is rewritten to the result, and the copy's descriptor (`child[0]`, `LmxCallable`) receives a hidden `offset` -- the number of result fields before the fields of its source operand; the method body addresses its lexical parent's fields as `node`, `offset + j`, and an unmerged occurrence has `offset = 0` ([§11](#call)). When `merge` encounters an `independent: const: immutable` value, it **does not copy** the branch: it places the original physical reference in the result, preserving identity and the source module as owner. This is a terminal of the common traversal, not another construction mechanism.
