...
- [x] Прозрачность единственного анонимного Structure-контейнера — свойство роли вызова / потребления receiver'ом, применяемое после разрешения головы, а не свойство дерева: одно и то же дерево `[head, body=[anonymous Structure()]]` есть `f: ()` (вызов, ноль аргументов) при callable-голове и `mystruct: ()` (объявление; Structure — задающее тип значение, не прозрачно) при отсутствующем имени. Аксессор последовательности аргументов применяют только потребители вызова (native и interpreter одинаково); именно эта роль и только она доказана в §2 («Proven for CALL-consumer role only», GROKBOT-CALLABLE-INTERP-STAGEC-20260922-65). Parser retains Structure for `mystruct: ()`; CALL unwrap remains Stage C (GROKBOT-P0-EMPTY-FORMS-20260922-74).
