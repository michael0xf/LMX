...
2. ~~**-203 массивы над вычисленным операндом**~~ — ПОСАЖЕН (`901ae2a` + `cab153c` + `ab1ecbe`; одна форма, `entry_argc_if` бежит с Entry 7, мутации измерены; REVIEW 901ae2a/8f8ad3a/ab1ecbe). Остаток класса: `c.puts`/`c.strcmp` в корне (Q7), `size_t → int` (конверсии §7). D-39 (индекс) ПОСАЖЕН, F-72. Ниже — прежнее описание: ELEM/ELEMPUT/LENGTH берут операнд-узел, дающий дескриптор массива (OF/DEREF/ELEM массива массивов), классификация по арене, **одна форма** (эмиссия 5a переводится на неё; 5 строк 5a — регрессионный пин); даёт 5 строк 5b (`length(m\mainArgs)`, `m\mainArgs[1][0]`); адреса `@ m\mainArgs[1][0]` в C-вызовах — класс Q7, отдельно. Ядро (`lmx_walk.lm1` :548-:571, армы 25–27) + транслятор (`l2_rw_elem*`, `l2_rw_opty`).
