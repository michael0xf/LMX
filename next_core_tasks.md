...
- [x] Единственный анонимный Structure-контейнер всего списка аргументов прозрачен; receiver потребляет его внутренние поля. **Proven for CALL-consumer role only** under GROKBOT-CALLABLE-INTERP-STAGEC-20260922-65 (`lmx_walk_call` + prepare scan mirror; empty ⇒ 0 args; OP_NONE fields pass-by-ref). Not tree-wide; declaration `mystruct: ()` untouched.
...
- [x] Не удалять и не переписывать `lm_p0_validate_nonempty_colon_frames*`. Перенести материализацию явно закрытого пустого тела (`dev/l1src_sandbox/parser.lm1:3459-3493`: при закрытии разделителем в пустое тело colon-владельца добавляется анонимная пустая Structure) во все копии parser и generated/self-build mirrors; с этим дочерним элементом существующий валидатор принимает явно закрытое `f:`/`---` и Frame с вертикальными детьми и по-прежнему отвергает незавершённое одиночное `f:` (EOF, `;`, `)`). `LM_P0_FRAME_SEPARATOR_CLOSED` означает закрытие `;`/`)` — висячую форму — и не читается как «явно закрыто». (GROKBOT-P0-EMPTY-FORMS-20260922-74)
- [x] Каноническое представление явно закрытой пустой вертикальной формы совпадает с `f: ()`: тело из одного анонимного пустого Structure-контейнера (представление §4.0.1 сохраняется); `f()` даёт тело без полей. Эквивалентность нульарных форм — семантическая, при потреблении receiver'ом (единственный анонимный контейнер прозрачен), а не побайтовая идентичность деревьев P0; goldens `tests/parser/current` 0–7 и 11 не меняются. Без явного `---` одна запись `f:` до транслятора не доходит. (GROKBOT-P0-EMPTY-FORMS-20260922-74)
- [ ] Прозрачность единственного анонимного Structure-контейнера — свойство роли вызова / потребления receiver'ом, применяемое после разрешения головы, а не свойство дерева: одно и то же дерево `[head, body=[anonymous Structure()]]` есть `f: ()` (вызов, ноль аргументов) при callable-голове и `mystruct: ()` (объявление; Structure — задающее тип значение, не прозрачно) при отсутствующем имени. Аксессор последовательности аргументов применяют только потребители вызова (native и interpreter одинаково); именно эта роль и только она доказана в §2 («Proven for CALL-consumer role only», GROKBOT-CALLABLE-INTERP-STAGEC-20260922-65). Parser retains Structure for `mystruct: ()`; CALL unwrap remains Stage C (GROKBOT-P0-EMPTY-FORMS-20260922-74). **Состояние (FABLE-P0-LANDED-VERIFY-20260922-77 / -85, HEAD 2300374): interpreter DONE (`lmx_walk.lm1` :288-308, :497-522); native ABSENT — `test: ()`, `test:`+`---`, `add: (1 2)` отказаны «incompatible entry signature» в `l2_check_primary` (`l2_actual_count` считает контейнер аргументом; три чтения `call\body\first_field` после разрешения головы: счёт, цикл проверки/admission, цикл эмиссии; typed fnptr — ещё два), в работе как FABLE-GROKBOT-CALL-ARGS-20260922-92 (один аксессор последовательности аргументов, без флагов форм); declaration ABSENT — `mystruct: ()` отказан «assignment target must be a declared typed mutable value» (`l2_check_body`, строка `l2_is_asgn`), отдельный тикет объявления по модели.**
...
- [ ] На уровне P0 проверить, что parenthesized, short/colon и vertical/block дают одну и ту же последовательность аргументов при потреблении (непустые списки — одинаковые прямые поля; нульарные — пустая последовательность с контейнером или без него); не размножать по этим формам ветки или таблицы транслятора.
