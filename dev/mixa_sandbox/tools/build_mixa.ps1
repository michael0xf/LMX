...
# Modeled on tools\build_l2src.ps1. All inputs -- translator, L2 core sources, kernel

# headers, and lm1/build -- must be local to this LMX repository tree. There is NO runtime
...
$kernelRoot = $l1Root
$lm1Root = $l1Root
# GUARD: fail BEFORE translation with the exact missing local path.  Skip only in

# ManagerLinkOnly mode, which reuses existing objects without re-translating.

if ($ManagerLinkOnly) {

    # Reuses existing stamp objects; -ReuseStamp check below validates the stamp dir.
    ...
    foreach ($m in $mustHave) {
        ...
        if (-not (Test-Path -LiteralPath $m.Path)) {
            ...
            throw "build_minta: missing local input -- $($m.Name) not found at $($m.Path). All inputs must be local; no external fallback."
