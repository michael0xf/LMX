...
9. **§7 порт implements/admission** — текущий пункт. T5–T7 посажены, REVIEW c713873 OK. План `steps/implements-s7.md`. Допуск после `l2_uses_walk_frame` — `l2_admit_paths`: Consumer — сам список путей, без `req` третьим аргументом и без `l2_ns_n`. Ноль путей успех (`unit_s7_empty` Entry 7), прочитанные имена сверяются (`unit_s7_used` Entry 7). 64 пути — Entry 7; 65-й — «a uses list is full». Кадра нет — дескриптор. Первый сегмент пути; вложенное имя — грубость S3. Таблица `primitive.convert` — данные Message, отбор не по имени, глобальный реестр не заводится. §8-blocking.
