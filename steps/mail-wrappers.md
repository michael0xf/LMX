...
## Уточнение (2026-09-25, Sonnet -172): именованные поля письма

`\[N]` в языке — номер вхождения имени, второго смысла (позиционный слот графа) не получает. Письмо — Structure с именованными полями модели письма, объявленной самой обёрткой (не пользователем): `sender` (`@: LmxMsg`) и `payload` (Structure). `receiveMessage: m` привязывает `m` к этой модели; `receiveMessage: m Model` типизирует `m\payload` моделью пользователя (допуск поля 1 на месте приёма); чтения — обычные пути: `m\sender`, `m\payload\mainArgs`; ответ — `sendMessage: m\sender reply(...)`. Транслятор синтезирует модель письма на месте приёма (как объявленную именованную Structure, без регистрации имени), «свой Message» (self) отправителем не является.

## Решение ядра (2026-09-25, Grok -177 к.5): исход допуска в receive-if

Выборочный приём `receiveMessage: m Model` (PRIM `lmx_walk_receive_if` над курсором inbox): `IMPLEMENTS_YES` — письмо берётся; `IMPLEMENTS_NO` — письмо пропускается (это «не то письмо», не отказ примитива), курсор идёт дальше; `IMPLEMENTS_UNKNOWN` и прочее — допуск не удостоверен → неявный throw `implements` вызывающего (`LMX_PRIMITIVE_THROW_IMPLEMENTS` → `LMX_WALK_THROWN + 2` в корне); пустой ящик или ни одного совпадения — `m = 0` без throw. Отказ merge-примитива (`Model: m`) — неявный throw `merge` (`THROWN + 1`). Числа — по S1.1: объявленные 1..d, merge d + 1, implements d + 2; в корне d = 0. (D-55, F-49.)
