...
Блочная, короткая и скобочная записи создают одну и ту же общую форму «голова принимает хвост»; пунктуация не определяет роль головы. После построения Frame роль выбирается по разрешённой голове в строгом порядке: (1) найденное callable-поле вызывается; (2) найденная non-callable привязка получает присваивание; (3) объявление возможно только через конструкцию, которая сама явно задаёт тип. Если имя головы отсутствует и такая объявляющая конструкция неприменима, это ошибка неизвестного имени/типа. Значение справа никогда не выводит тип новой переменной: в языке нет `var`. Само двоеточие, скобки или переход уровня не объявляют, не присваивают и не вызывают. Одноимённое заранее подготовленное физическое место не считается активной логической привязкой.

`f()` само по себе не означает вызов: это Frame с головой `f` и пустым Structure-телом, синтаксически равный `f: ()` и явно закрытому пустому вертикальному телу. Если `f` разрешено как callable, получается нульарный вызов. Если `f` уже существует как non-callable привязка, применяется обычное присваивание пустой Structure с `implements`/admission. Если `f` отсутствует, пустое Structure-тело само явно задаёт структурный тип и объявляется новая пустая структурная переменная `f`. Так `f()` при отсутствующем `f` — объявление, а не неудавшийся вызов; ни одна из трёх операций не выбирается по скобкам. При найденном callable ошибка его сигнатуры остаётся ошибкой вызова без перехода к следующим правилам.
...
`f()` does not mean a call by itself: it is a Frame headed by `f` with an empty Structure-body, syntactically equivalent to `f: ()` and an explicitly closed empty vertical body. If `f` resolves to a callable, it is invoked without arguments. If `f` already has a non-callable binding, ordinary assignment of the empty Structure applies, with `implements`/admission. If `f` is absent, the empty Structure-body itself explicitly supplies a structural type and declares a new empty structural variable `f`. Thus `f()` with an absent `f` is a declaration, not a failed call; parentheses select none of the three operations. Once a callable is found, a signature error remains a call error without falling through to the later rules.
...
В исполняемом теле безголовая запись — обычное выражение, а не ошибка и не особая форма вызова. Это относится к одиночному `2`, голому `f`, последовательности `2 + 2`, анонимной структуре `(f)` или `(2 + 2)` и пустой `()`. Их поля разрешаются и вычисляются в лексическом порядке тем же механизмом выражений; результат, которому не указан получатель, отбрасывается. Анонимная структура с несколькими полями, в том числе с Frame и безголовыми выражениями, сохраняет порядок исполнения всех полей; ограниченная и соответствующая вертикальная запись не выбирают разные операции. Если разрешённый `f` является callable, голое `f` вызывает его нульарно по общему приоритету вызова; non-callable `f` только вычисляется. Литерал `2` не объявляет тип для `f: 2`. Пустая `()` допустима как присутствующая пустая Structure без внутренних выражений. Ни форма скобок, ни факт отбрасывания результата не обходят обычное разрешение, admission или диагностику неизвестного имени.
...
Общий физический дескриптор Array — `VoidArray {size_t size, void *data}`; `LmxArrayDesc` является псевдонимом этого типа. В `Lmx {Lmx *parent, VoidArray array}` тот же дескриптор вложен по значению и является самим массивом дочерних физических ссылок; его backing зарегистрирован как адресный диапазон арены, но запись диапазона не является самим `VoidArray`. У самостоятельного Array `size` — логическое число элементов, backing содержит ровно столько ячеек. У базового Array нет `capacity`, и он не растёт, не делает resize/append и не переключает backing. Динамическое членство, ёмкость и рост принадлежат только отдельному List (`KIND_LIST`), у которого ёмкость живёт во внутреннем префиксе backing. Физические адреса объектов-ссылок от роста List не зависят; адрес ячейки закрытого backing List не сохраняется через его замену.
...
The common physical Array descriptor is `VoidArray {size_t size, void *data}`; `LmxArrayDesc` aliases that type. In `Lmx {Lmx *parent, VoidArray array}`, the same descriptor is embedded by value and is the actual array of physical child references; its backing is registered as an arena address range, but the range entry is not the `VoidArray` itself. For a standalone Array, `size` is its logical element count and the backing has exactly that many cells. Base Array has no `capacity` and does not grow, resize, append, or switch backing. Dynamic membership, capacity, and growth belong only to a separate List (`KIND_LIST`) whose capacity lives in a private backing prefix. Physical addresses stored as referent values are unaffected by List growth; a List backing-slot address does not survive private-backing replacement.
