...
2. **-203 массивы над вычисленным операндом** — ядро ПОСАЖЕНО (`901ae2a`, одна форма, гейт 276/398/11); **транслятор частично**: 3 строки 5b с `receiveMessage: m` (прямой приём в объявленную Structure) всё ещё `an array` — `steps/review-log.md` REVIEW 901ae2a п.2; RESULT в план дописать (п.1). (§9 tickets; к.1 план `steps/array-operand-203.md`): ELEM/ELEMPUT/LENGTH берут операнд-узел, дающий дескриптор массива (OF/DEREF/ELEM массива массивов), классификация по арене, **одна форма** (эмиссия 5a переводится на неё; 5 строк 5a — регрессионный пин); даёт 5 строк 5b (`length(m\mainArgs)`, `m\mainArgs[1][0]`); адреса `@ m\mainArgs[1][0]` в C-вызовах — класс Q7, отдельно. Ядро (`lmx_walk.lm1` :548-:571, армы 25–27) + транслятор (`l2_rw_elem*`, `l2_rw_opty`).
