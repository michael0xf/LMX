...
Чтобы изменение увидели другие держатели значения, запись выполняется через явный путь: `p\x: value` или `a[i]: value` меняет выбранный референт. Голое `x: value` изменяет рабочее значение текущей активации: обычное own-поле становится `dirty` до успешной контрольной точки, а обычная формальная, результатная или динамическая копия остаётся локальной. Узкое исключение для адреса входа до одноимённой own-привязки и момент его активации определены в [рабочем состоянии](#dynamic). Ни одна из этих записей не добавляет одноимённое вхождение неявно.
