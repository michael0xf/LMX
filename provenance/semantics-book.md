...
The common physical Array descriptor is exactly `{len, data}`: logical element count and the address of a backing that has exactly `len` cells. Base Array has no `capacity` and does not grow, resize, append, or switch backing. Dynamic membership, capacity, and growth belong only to a separate List (`KIND_LIST`) whose capacity lives in a private backing prefix. Physical addresses stored as referent values are unaffected by List growth; a List backing-slot address does not survive private-backing replacement.
