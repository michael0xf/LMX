...
9. **§7 порт implements/admission** — текущий пункт. T5–T7 посажены, REVIEW c713873 OK. План `steps/implements-s7.md`. Срез 1 посажен: identity вызывает `l2_descriptor_implements` и при `cand = req` остаётся успехом (`unit_s7_identity` Entry 7). Пустой `uses` остаётся успехом. Таблица `primitive.convert` — данные Message, глобальный реестр не заводится. §8-blocking.
