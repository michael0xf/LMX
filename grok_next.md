...
4. **-200 POSIX-двойник** — срезы (а), (б) и остаток REVIEW b5e72d7 ПОСАЖЕНЫ (`5bc8ec7`; машина 280/399/11; **облако fable: `build_l2src.py` 222/233, L3 11/11**). Ядро вне тел `_win32` Win32 не зовёт. **Следующий код — REVIEW 5bc8ec7 п.1–п.2:** Win32 в девяти селфтестах (`c.Sleep` → `pause_ms`; потоки `lmx_post_mpsc`/`lmx_app_running_lane` — тот же контракт заголовок+два тела, без третьего списка имён; `<direct.h>`/`_mkdir` в двух `lmx_root_*` — слово хоста рядом с `lmx_root_os_cwd`); цель облака 233/233. D-78 отдельно, после этого среза.
5. **T4b** — классы 1 и 2 ПОСАЖЕНЫ (`b5a2cf9` и formal binding, `steps/walk-methods-t4b.md`, harness 400/400, дифф 216/216). Класс 3 (own-поле во вложенном теле) — после среза п.4. Throws и callable-формалы ждут своего места.
