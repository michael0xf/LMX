...
- [ ] `f()`, `f: ()` и явно пустая вертикальная форма `f:` с обязательным `---` должны иметь **одинаковое P0 дерево**: Frame с пустым Structure-body. Общая §4.0.1 действует и для непустой единственной анонимной обёртки всего списка: `f(a b)` = `f: (a b)` уже после P0. Незавершённое `f:` P0 отвергает. Голый `f` остаётся atom/expression, но для callable даёт тот же нульарный эффект. Полный аудит и маршрут — `next_parser_fix.md`.
...
- [ ] Для непримитивного нового значения полностью выделить требуемую память/графовую структуру согласно его конструктору; неизвестный тип — ошибка. **Аудит A1 §3 (Sonnet, FABLE-SONNET-UNIT-FIELD-WRITE-20260923-129 ч.2, по факту на `765e70c`):** полное построение известных не-примитивов — фикстуры Model (-105/S2); «неизвестный тип → ошибка» измерено (located «assignment target must be a declared typed mutable value»), но не закреплено → фикстура в тикете -130.
...
- [ ] Если разрешённая голова является callable-полем, всегда выбирать вызов с высшим приоритетом — независимо от `COMPACT`, colon или блочной формы. **Аудит A1 §3 (Sonnet, FABLE-SONNET-UNIT-FIELD-WRITE-20260923-129 ч.2, по факту на `765e70c`):** та же фикстура, что :181 (тикет -130).
- [ ] Если сигнатура или admission такого вызова не принимают аргументы, выдавать ошибку вызова без fallback к assignment/declaration. **Аудит A1 §3 (Sonnet, FABLE-SONNET-UNIT-FIELD-WRITE-20260923-129 ч.2, по факту на `765e70c`):** блочная/скобочная формы — отказ без fallback доказан (`entry_bad_sig.lm2`, `unit_call_args_refuse_named.lm2`); colon-форма не закреплена → в -130: существующий метод `foo` без аргументов, `foo: 5` с телом → located-отказ по арности вызова, не «assignment target».
- [ ] Универсальный приоритет по resolved binding (не name/syntax exception и не fallback): callable ⇒ call; существующий non-callable ⇒ assignment; отсутствующее имя ⇒ declaration только когда тип явно задан конструкцией, иначе ошибка. Отдельно исправляется unsupported terminal Structure assignment-path. **Аудит A1 §3 (Sonnet, FABLE-SONNET-UNIT-FIELD-WRITE-20260923-129 ч.2, по факту на `765e70c`):** приоритет — тикет -130 (см. :181); подпункт «unsupported terminal Structure assignment-path» — это остаток -92: statement-level `ty40` отказ в `l2_check_body` (тогда :12054/:12059) для присваивания Structure-значения по пути; измеряется и закрывается в -131 вместе с :207.
...
- [ ] Не поддерживать прямое присваивание callable-полю. Его замена допускается только общими механизмами: `merge` структур с `implements`, динамической перегрузкой одноимённым callable вызывающего либо явной передачей callable в дескрипторе аргументов вызывающего. **Аудит A1 §3 (Sonnet, FABLE-SONNET-UNIT-FIELD-WRITE-20260923-129 ч.2, по факту на `765e70c`):** архитектурно так (`l2_head_is_call`), отрицательной фикстуры нет → в -130 (`foo: 5` против `foo`, не принимающего аргумент: отказ вызова, не присваивание callable-полю).
