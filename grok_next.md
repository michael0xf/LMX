...
4. **-200 POSIX-двойник** — срезы (а), (б) и остаток REVIEW b5e72d7 ПОСАЖЕНЫ (`5bc8ec7`; машина 280/399/11; **облако fable: `build_l2src.py` 222/233, L3 11/11**). Ядро вне тел `_win32` Win32 не зовёт. **Следующий срез — REVIEW 5bc8ec7 п.1–п.2:** Win32 в девяти селфтестах (`c.Sleep` → `pause_ms`; потоки селфтестов `lmx_post_mpsc`/`lmx_app_running_lane` — тот же контракт заголовок+два тела; `<direct.h>`/`_mkdir` в двух `lmx_root_*` селфтестах — слово хоста рядом с `lmx_root_os_cwd`); цель облака 233/233. D-78 (`lmx_post_sweep` на POSIX, `steps/defects.md`) — отдельно.
5. **T4b** — класс 1 (Structure-формал) ПОСАЖЕН (`b5a2cf9`, `steps/walk-methods-t4b.md`). **Следующий код — класс 2, formal binding** (own-поле с именем формала). Дальше own-поля во вложенном теле; throws и callable-формалы ждут своего места.
