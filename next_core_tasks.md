...
- [x] **IMPLEMENT cross-ref (post-Stage1; AUTHOR-CALLABLE-DESCRIPTOR-IMPLEMENT-20260922-43):** same cases as §2 — `test2`/`test4` formals + bare `return: f` (once KIND=CALLABLE, no special return rule); E2E `test2(test)==1`, `test4(test)==1`; direct `test3` → NOT_CALLABLE without inventing a body. Native DONE CONTINUE-54 Stage B; interpreter not claimed.
